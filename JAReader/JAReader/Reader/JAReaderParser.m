...
+ (CTFrameRef)parserContent:(NSString *)content
    ...
    NSDictionary *attribute = [[JAReaderConfig defaultConfig] attributes];;
    ...
    // 一个保存着样式,样式作用的内容区间,内容的边界的结构体指针 frameRef
...
+(CFIndex)parserIndexWithPoint:(CGPoint)point frameRef:(CTFrameRef)frameRef
{
    ...
    if (lineCount) {
        ...
        for (int i = 0; i<lineCount; i++) {
            ...
            CGRect lineFrame = CGRectMake(baselineOrigin.x, CGRectGetHeight(bounds)-baselineOrigin.y-ascent, lineWidth, ascent+descent+linegap+[JAReaderConfig defaultConfig].lineSpace);    //没有转换坐标系左下角为坐标原点 字体高度为上行高度加下行高度
...
+ (NSString *)parseMarkup:(NSString *)markup {
    NSError *err = nil;
    NSRegularExpression *re = [NSRegularExpression regularExpressionWithPattern:@"(.*?)(<[^>]+>|\\Z)" options:NSRegularExpressionCaseInsensitive | NSRegularExpressionDotMatchesLineSeparators | NSRegularExpressionAllowCommentsAndWhitespace error:&err];
    if(err) {
        NSLog(@"%@",err);
    }
